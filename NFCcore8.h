...
#define SerialSpeed 115200     // Serial通信時のスピード/ArduinoUNO,MEGA,Micro等は9600(デフォルト),ESP32は115200
...
class NFCcore8 {
  ...
  public:
    ...
    void show_cardInfo();
    bool set_cardInfo();
    bool updateSavedTime();  //ブロック1。呼び出し前にpublic内のDate[]を更新する必要あり。
  ...
  public:
    ...
    String userName = "";  //初期値
    bool ApplePay = false;  //waitTouch()の際に更新」。ApplePay端末がかざされたかの確認
    byte blockReadData[16];
    byte sectorReadData[3][16];
    byte blockWriteData[16];
    byte sectorWriteData[3][16];
    byte uid[4];
    byte Date[6] = {0, 1, 1, 0, 0, 0}; //デフォルト[年,月,日,時,分,秒]!!ただし年は1970年からの経過年数を用いる
  ...
  protected:
    const byte miliInterval = 50;  //waitTouch()内でのカードの読み取り頻度。推奨50ms
    bool dumpInfo = true;  //各関数内でSerial.print()を行うか否か。組み込みの際は表示なしが推奨
    ...
    MFRC522::MIFARE_Key KeyA = {keyByte: {255, 255, 255, 255, 255, 255}};  //デフォルトキー
    MFRC522::MIFARE_Key KeyB = {keyByte: {255, 255, 255, 255, 255, 255}};  //デフォルトキー
